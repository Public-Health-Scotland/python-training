...
Read in the `borders_inc_age` csv file. Create a new column called `number_of_eyebrows` with the row_value as `2` for all the rows.
...
This method uses the boolean operator `==`, which means **equal to**.

This operator can be substituted with others.

For example, in the case that you wish to find all the values which are less than `5` in a given column, and replace them with `Small`, you would code the following:
...
```{python recode, exercise = TRUE, exercise.setup = "pandas-setup"}
...
```{python recode-hint-1}

borders_data = pd.read_csv("data/borders_inc_age.csv")

borders_data.loc[borders_data[...] = ...
```

```{python recode-hint-2}

borders_data = pd.read_csv("data/borders_inc_age.csv")

borders_data.loc[borders_data['HospitalCode'] == ..., ...] = 'Replaced'
```

```{python recode-hint-3}

borders_data = pd.read_csv("data/borders_inc_age.csv")

borders_data.loc[borders_data['HospitalCode'] == 'B120H', 'HospitalCode'] = 'Replaced'
```

```{python recode-solution}
...
The column `HospitalCode` in `borders_data` doesn't follow the style guide, rename it to `Hospital_Code`. 


```{python rename, exercise = TRUE, exercise.setup = "pandas-setup"}
...
```{python rename-hint-2}

borders_data = pd.read_csv("data/borders_inc_age.csv")

borders_data.rename(columns={...})
```

```{python rename-hint-3}

borders_data = pd.read_csv("data/borders_inc_age.csv")

borders_data.rename(columns={'HospitalCode':'Hospital_Code'})
```

```{python rename-solution}
...
You should further define what common columns/variables the data frames should merge `on` by defining the `on = ['<insert_column>']`
...
```{python merge, exercise = TRUE, exercise.setup = "pandas-setup"}
